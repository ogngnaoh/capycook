...
| S8 | Release: reviewer-ready GitHub repository + packaged current media | shipped (2026-07-14; Task 7 independently reviewed and approved; pushed to private `origin/master`) | `docs/superpowers/plans/2026-07-14-milestone-02-s8-showcase-release.md` |
...
- (m) **S8 shipped (2026-07-14).** The final handoffs, milestone archives, live oracle consumer paths, and exact USER-authorized administrative PREREGISTRATION relocation row landed as one atomic ship commit on private `master`, then pushed. Task 7 (private release) was independently reviewed and approved; that review earned its keep by catching a **High** finding — the frozen-PREREGISTRATION guard passed *vacuously*, comparing already-pushed remote `master` against the same checked-out commit, so it could never observe that the ship commit changed `docs/PREREGISTRATION.md`. The repair replaced the moving-ref comparison with fixed pins (`FROZEN_BASELINE` ancestry + an ordered authorized-blob allowlist). Public visibility remains a separate USER-authorized gate; milestone 03 is parked and no milestone is active.
