...
- [ ] **Step 1 [AUTHOR]: Blind-label the sample.** Open `eval/out/blind_check.csv`; for each row fill the `label_r1` column with one of the five frozen §7a categories, judging only `dish`/`text`/`source`. Do NOT open `blind_check_map.csv` first.
- [ ] **Step 2: Score it.**
...
- **Reliability plan (as amended):** PREREG §9 Amendment 1 replaced the second human
  ...
  validated by an author blind-check control), plus a blinded author R1 pass and a
  DeepSeek judge R2 with pre-adjudication Cohen's κ over all Tier-2 claims. Amendment 2
  added bounded move retries to the harness runner after live-model variance made the
  all-or-nothing arm design infeasible (full text + rationale in
  [docs/PREREGISTRATION.md §9](docs/PREREGISTRATION.md)).
```
...
The blind-check control — the author blind-labeling an 18-row stratified Tier-1 sample —
agreed with the verifier on **<AGREEMENT>**.

### Gate dynamics — H2

<PASTE the `eval replay` table from Task 5, with its explicit N and single-operator
caveat verbatim.>
...
`[unverified]`. The arms therefore separate not on error rates but on **citation
uptake**, which was low everywhere (10/209 grounded, 10/203 flavorgraph, 0/150
ungrounded): grounding evidence changed *whether* the model cites (ungrounded cited
nothing; both evidence-fed arms cited, always correctly) but not how often it grounds a
claim. Tier-1's 100% coverage — a consequence of provenance vocabulary staying within
`pairing:`/empty — collapsed the planned human-labeling campaign to the 18-row control.
These are descriptive results over one model, one seed set, and a single-operator
telemetry stream; they are not user-research or quality claims (§8).
```
...
**Files:**
- Modify: `README.md` (the `<PASTE ...>` H2 slot from Task 4), `docs/02-measure-run/log.md`
...
- [ ] **Step 2: Fold.** `go run ./cmd/eval replay` — paste the emitted markdown table + caveat verbatim into the Task-4 slot. If N is still thin at execution time, it ships with its honest N per §8 — never wait indefinitely; the user decides when to stop accumulating.
...
- [ ] **Step 3:** milestone.md: S6 → shipped, S7 → next; handoff.md overwritten; one log line. Commit.
