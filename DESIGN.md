...
- **Persistence:** the version-chain draft store (snapshots + diffs + branch refs); the **move/gate event log** — an append-only, *truly event-sourced* stream that is the eval data source and is replayed to compute metrics (event types now include **`move_cancelled`** and **`proposal_blocked`**, so cancel/block behavior is visible to the gate-dynamics hero metric); user profile (skill, equipment, dietary, taste history) *[annotated 2026-07-06: P1+/R2-scoped, not v0 — v0 persists constraints per-dish only, and "taste history" is bounded by §16.1 R2's per-project silo]*; a flat vector index for the FlavorGraph signal (a similarity lookup — no separate graph database, since no multi-hop pairing traversal is planned).
...
- **Anaerobic preservation block.** Any proposal involving low-oxygen environments — oil infusions, vacuum/sous-vide-style anaerobic holds, home canning, fermentation, curing — is **hard-blocked in v0** (locked v0-scope decision #3; block-with-citation-override is a P1 refinement) *[amended 2026-07-06 — the earlier "flagged for mandatory safety-citation or refusal" wording predated the lock]* (the raw-garlic-in-room-temp-oil → *Clostridium botulinum* vector is the textbook case, and a real reported 2024 LLM failure).
...
- **(a) Per-claim provenance/hallucination** — nominal categories *grounded-correct · correctly-unverified* (count FOR the system) *· grounded-mischaracterized* (its own bucket — counts neither for nor against; the frozen PREREGISTRATION §7a refinement governs) *· hallucinated · opinion/non-checkable* (excluded) *[amended 2026-07-06 to match PREREGISTRATION §7a]* → scored with **Cohen's kappa** (RAGTruth/FaithBench style — arxiv.org/html/2401.00396v1, arxiv.org/html/2410.13210v1).
...
> **Amended 2026-07-06:** sequencing below is superseded by the end-to-end build spec (milestone 01); the exit criteria stand and are re-homed by that spec §2/§3. The v2 "live demo" deliverable is re-scoped to a fork-friendly repo + demo GIFs/runbook (spec §1.5).

| Phase | Goal | Exit criterion |
|---|---|---|
| **v0 — Scaffold + pre-register (wk 0–1)** | Repo skeleton; vendor FlavorGraph, load USDA/FoodOn; **stand up the eval harness shell + tracing**; sketch the safety blocklist; **write the README methodology + hypothesis before any run**; stand up the walking skeleton (S0.4): thin UI → Go-served → containerized | The 3-arm harness runs an empty baseline; tracing emits a replayable event; **README pre-registers the ablation and what a null means** — no external subjects; the walking skeleton serves a graybox workbench from the Go binary in a container |
| **v1 — Pillar 1: the loop (wk 2–6)** | P0-A + P0-1, P0-5, P0-6, P0-7, P0-7b, P0-9: hand-rolled gated loop + version chain + idempotent accept + deterministic services + cuisine-aware grounded capability with baseline toggle + safety gate + SSE/cancel + minimal autonomy dial; build the two-pane workbench UI as the loop's interaction surface | A cook takes a seed to a finished, grounded, costed, *safety-screened* dish entirely through gated moves; both ablation arms run; an unsafe proposal is blocked |
| **v2 — Pillar 2: measure + iterate + deploy (wk 7–10)** | P0-B + P0-8, P0-11: hero process metrics + 3-arm ablation + versioned benchmark + inter-rater spot-check; post-cook iteration; provenance/safety overlays on the existing workbench; deploy hardening + eval-results surface (P0-11 re-scoped from "the frontend" to overlays, since the workbench ships in v1) | README results table led by **process metrics** (provenance/hallucination + gate dynamics), grounding ablation in a supporting section; live demo |
